  ...
  return (
    <>
      ...
      <main className="w-full min-h-screen h-screen flex">
        <section className="h-full border-r hidden w-[15%] lg:block relative overflow-hidden">
          <Sidebar />
        </section>
        <section className="h-full main-column lg:w-[85%] w-full">
          <NavBar />
          <MainBody />
        </section>
      </main>
...
const Sidebar = () => {
  ...
  return (
    <div>
      <header className="w-full h-16 border-b flex items-center pl-3">
        <p className="text-[3rem] font-medium">Do-To<span className="text-blue-400">.</span></p>
      </header>
      <section className="h-auto w-full px-3 pr-1 pt-6 ">
        <div className="w-full flex items-center gap-1 cursor-pointer">
          <h1 className="font-semibold text-[16px] text-[#222]">
            My projects
          </h1>
        </div>
        <AnimatePresence>
...
const MainBody = () => {
  ...
  return (
    <main>
      <header className="w-full h-20  pl-7 flex justify-between border-b border-zinc-200">
        <ProjectTitle />
        <div className="h-full hidden lg:flex items-center gap-4 px-6">
          <div>
            <input
              type="text"
    ...
    <div className="h-full flex justify-center flex-col gap-1">
      <AnimatePresence>
        {!isEditingName ? (
          ...
          >
            <p className="text-4xl max-w-96 truncate font-poppins font-semibold text-[#222]  leading-[44px]">
          ...
          >
            <input
              ...
              className="text-3xl font-semibold pr-0 border w-auto border-zinc-500 pl-2 outline-none -translate-x-[10px]"
            />
            <div
              className="cursor-pointer"
              onClick={() => {
...
const TasksBody = () => {
  ...
  return (
    <main className="w-full min-h-[55vh]">
      <DragDropContext
        onDragEnd={(result) => onDragEnd(result, columns, setColumns)}
      >
        <section className="w-full h-[84vh] lg:grid flex grid-cols-4  task-grid overflow-y-hidden overflow-x-scroll">
          {Object.entries(columns).map(([columnId, column], index) => {
            ...
            return (
              <div
                key={index}
                className=" h-full lg:w-full min-w-[450px] lg:min-w-0 p-2"
              >
                <div className="w-full h-full border task-column border-zinc-200 px-2 pt-2 rounded-lg flex flex-col gap-2">
                  <div className="flex w-full h-6  items-center justify-between pl-2">
                    ...
                    {(provided, snapshot) => {
                      return (
                        <div
                          ...
                          className="w-full max-h-[75vh] min-h-[10rem] tasks-container p-2 bg-zinc-200 rounded transition-all duration-200 ease flex flex-col gap-2 overflow-auto"
                        >
